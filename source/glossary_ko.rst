...
.. glossary::
    ...
    바이너리 배포판(Binary Distribution)

        컴파일된 확장 모듈을 포함하는 :term:`빌드 배포판(Built Distribution)`.


    빌드 배포판(Built Distribution)

        설치하기 위해 올바른 위치에 옮기기만 해도 되는 파일과 메타데이터를 포함하는
        :term:`배포판 <배포용 패키지(Distribution Package)>` 포맷.
        :term:`Wheel`\ 은 여기에 포함되지만
        :term:`소스 배포판 <소스 배포판(Source Distribution or "sdist")>`\ 은 설치되기전에 빌드 단계가 필요하므로
        여기에 포함되지 않는다.
        이 포맷은 Python 파일들이 이미 컴파일(pre-compile)되어야 한다는 뜻은 아니다.
        (:term:`Wheel` 은 일부러 컴파일된 파이썬 파일들을 포함하지 않는다)


    배포용 패키지(Distribution Package)

        파이썬 :term:`package <임포트용 패키지(Import Package)>`,
        :term:`모듈 <모듈(module)>`, 그리고 다른 :term:`Release`\ 를
        배포하기 위한 리소스 파일들을 포함하는 버전관리되는 아카이브.
        아카이브 파일은 최종 사용자가 인터넷에서 다운로드 하고 설치하는데
        쓰인다.

        배포용 패키지(Distribution Package)라는 용어 대신에 그냥 "패키지" 또는
        "배포판"으로 불린다.
        다만 여기에서는 배포용 패키지와 마찬가지로 그냥 "패키지"라고
        불리는 :term:`임포트용 패키지(Import Package)`\ 라든가
        "배포판"으로 쓰이는 다른 배포판(리눅스 배포판이나 파이썬 언어 배포판)과의 혼동을
        방지하기 위해서 필요하면 정확한 용어를 쓰기로 한다.

    Egg

        :ref:`setuptools`\ 에서 도입된 :term:`빌드 배포판(Built Distribution)`.
        현재에는 :term:`Wheel` 로 대체되고 있다.
        자세한 사항은 `파이썬 Eggs의 내부구조
        <https://setuptools.readthedocs.io/en/latest/formats.html>`_\ 와
        `파이썬 Egg <http://peak.telecommunity.com/DevCenter/PythonEggs>`_\ 를 참조.

    확장 모듈(Extension Module)

        파이썬에서는 C/C++, Jython에서는 자바 등 하위 레벨 언어로 구현된
        파이썬 :term:`모듈(module)`.
        주로 동적으로 로드할 수 있도록 미리컴파일된 파일에 포함된다:
        유닉스에서는 shared object (.so) 파일, Windows에서는 .pyd확장자를
        가지는 DLL 파일로 구현되고, Jython에서는 Java class이다.


    Known Good Set (KGS)

        서로 호환되는 버전을 가진 배포판의 모음. 특정한 패키지 모음이 제대로
        작동하는지 확인하기 위해 보통 여러 테스트를 통과해야 한다. 이 용어는 일반적으로
        여러 배포판으로 이루어진 프레임워크나 툴킷에 의해 쓰인다.


    임포트용 패키지(Import Package)

        다른 모듈을 포함하거나 재귀적으로 다른 패키지를 포함 할 수 있는 파이썬 모듈

        임포트용 패키지도 일반적으로 "패키지"라고도 불리나, 이 가이드는 혼동을 방지하기
        위해 정확한 용어를 사용한다.

    모듈(Module)

        파이썬에서 재사용이 가능한 코드 단위. :term:`순수 모듈(Pure Module)` 과
        :term:`확장 모듈(Extension Module)` 두가지 유형이 존재한다.


    Package Index

        :term:`package <배포용 패키지(Distribution Package)>` 의 발견과 사용을 자동화 시키기
    ...
    Project
        ...
        는 하나 이상의 :term:`배포판 <배포용 패키지(Distribution Package)>` 을 포함한다.
    ...
    Release
        ...
        Release를 만드는 것은 여러 :term:`배포판 <배포용 패키지(Distribution Package)>` 의
    ...
    Requirement
       ...
       :term:`Package <배포용 패키지(Distribution Package)>` 을 설치하기 위한 specification.
    ...
    Working Set
        ...
        Import를 위해 제공되는 :term:`배포판 <배포용 패키지(Distribution Package)>` 모음.
        `sys.path` variable상에 있는 distribution이다. 하나의 working set에는
        project를 위한 :term:`배포판 <배포용 패키지(Distribution Package)>` 하나만
        허용된다.
